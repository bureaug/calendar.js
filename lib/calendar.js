...
Calendar.prototype = {
    ...
    monthDates : function monthDates(year, month, dayFormatter, weekFormatter) {
        if ((typeof year !== "number") || (year < 1970)) {
            console.warn('For years below 1970 the dates are not reliable for the following reasons: 1) Daylight saving time (DST) is not taken into account; 2) Leap years might not be calculated correctly.');
        };
        ...
        var weeks = [],
            week = [],
            i = 0,
            givenDate = new Date(1970, month, 1);
			givenDate.setFullYear(year);
        var  date = this.weekStartDate(givenDate);
        do {
            for (i=0; i<7; i++) {
